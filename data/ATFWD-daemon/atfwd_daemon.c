...
char* getTargetFromSysProperty()
{
    ...
    if (!strcmp(ATFWD_DATA_TARGET_MDM, target)
        ...
        if (!strcmp(ATFWD_DATA_TARGET_MSM8994, msm_tgt)) {
            strlcpy(target, ATFWD_DATA_TARGET_MSM8994, ATFWD_DATA_PROP_SIZE);
        }
    }
...
char* getDefaultPort()
{
    ...
    } else if (!strncmp(ATFWD_DATA_TARGET_MSM8994, target, strlen(target))) {
        /* MSM8994 would uses first conn id */
