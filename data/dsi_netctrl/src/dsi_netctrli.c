...
static int dsi_process_target()
{
  ...
  do {
    ...
    {
      DSI_LOG_DEBUG("%s", "Loading Tesla LE config from xml");
      if(0 != dsi_config_load(DSI_CONFIG_FILE_PATH, target_str))
      {
        DSI_LOG_ERROR("%s: Unable to load Tesla LE config from xml", __func__);
        break;
      }
    }
    ret = DSI_SUCCESS;
...
/*!
@brief
  Configures rmnet_data support based on android property
