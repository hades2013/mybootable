...
/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2008,2010,2011,2013-2015 Qualcomm Technologies, Inc.
...
)
{
  ...
  /* Close stream and print return code */
  ds_log_low("%s(): Return code [%d]", __func__, WIFEXITED( pclose(stream) ));
...
pid_t ds_getpid(const char *pname)
{
  ...
  if (NULL == pname)
  {
    ds_log_err("%s:Bad input params\n", __func__);
    return ret_pid;
  }
  if(NULL == (dp = opendir("/proc")) )
  {
    ds_log_sys_err("open /proc failed");
    return ret_pid;
  }
  else
  {
    ...
    while(NULL != (dptr = readdir(dp)) )
    {
      errno = 0;
      /*
      *  Convets the string pid from /proc/<pid>/stat to integer.
      *  The checks post strtoul() makes sure that the string
      *  consists of only digit characters and that the conversion
      *  of all the digit characters to int was successful, and
      *  that there was no overflow determined after conversion/ERANGE.
      */
      pid_t pid = (pid_t) strtoul(dptr->d_name, &endptr, 10);

      if (errno == 0 && *endptr == '\0')
      {
        char file[DS_UTIL_MAX_STRING_SIZE] = "";
        char buf[DS_UTIL_MAX_STRING_SIZE] = "";
        char *ptr = NULL, *name = NULL;
        char *save_ptr = NULL;

        snprintf(file, sizeof(file), "/proc/%s/stat", dptr->d_name);

        fp = fopen(file, "r");
        /* We start iterating from pid 0. So its ok if we see failures when we try to access all
         * the other files except for the one corresponding to netmgrd */
        if (!fp)
        {
          ds_log_med("%s:Failed to open file=%s", __func__, file);
          continue;
        }

        if (!fgets(buf, sizeof(buf), fp))
        {
          ds_log_err("%s: Failed to read contents of file=%s", __func__, file);
          goto bail;
        }
        ...
        if (!ptr)
        {
          ds_log_err("%s:strtok_r failed", __func__);
          goto bail;
        }
        ...
        if (!strcmp(ptr, dptr->d_name))
        {
          char *name = strtok_r(NULL, " ", &save_ptr);
          char proc_name[DS_UTIL_MAX_STRING_SIZE] = "";

          if (!name)
          {
            ds_log_err("%s:strtok_r failed - not able to get pname", __func__);
            goto bail;
          }

          snprintf(proc_name, sizeof(proc_name), "(%s)", pname);

          if (!strcmp(name, proc_name))
          {
            ds_log_low("%s: Match found pname=%s pid=%d", __func__, pname, pid);
            ret_pid = pid;
            break;
          }
        }
