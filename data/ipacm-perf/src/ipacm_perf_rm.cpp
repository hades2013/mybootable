...
int ipacm_perf_rm::bw_vote()
{
	...
	/* if modem DL tput is LTE cat 11 and there is one monitored iface up, vote for high BW (900Mbps)*/
	if(has_iface_up && modem_configured_throughput.dl_configured_throughput >= IPACM_PERF_RM_LTE_CAT11_THRESHOLD)
	{
		bw_mbps = IPACM_PERF_RM_LTE_CAT11_BW_VOTE_MBPS;
	}
	else /* otherwise vote for low BW (100Mbps) */
	{
		bw_mbps = IPACM_PERF_RM_NON_LTE_CAT11_BW_VOTE_MBPS;
	}
#if 0
