...
-----------------------------------------------------------------------------*/
      if (tmgi_status == FALSE)
      {
        ...
        for (n=0; n < qmi_ip_conf->tmgi_instance[l].ref_count; n++)
        {
          ret_val = qmi_ip_tunnel_deletion(qmi_ip_conf->tmgi_instance[l].client_fds[n],
...
int qmi_ip_qmi_client_init()
{
  ...
  qmi_ip_wds_init(QMI_CLIENT_QMUX_RMNET_INSTANCE_0);
  qmi_ip_nas_init();
  qmi_ip_netmgr_init();
