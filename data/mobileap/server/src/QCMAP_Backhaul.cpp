...
//Create object for Firewall and NATALG here
    ...
    if(token)
    {
      ...
      if (strncmp(token,WWAN_BACKHAUL,strlen(WWAN_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_first = BACKHAUL_TYPE_WWAN;
      else if (strncmp(token,USB_CRADLE_BACKHAUL,strlen(USB_CRADLE_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_first = BACKHAUL_TYPE_CRADLE;
      else if (strncmp(token,WLAN_BACKHAUL,strlen(WLAN_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_first = BACKHAUL_TYPE_AP_STA_ROUTER;
      else
    ...
    if(token)
    {
      ...
      if (strncmp(token,WWAN_BACKHAUL,strlen(WWAN_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_second = BACKHAUL_TYPE_WWAN;
      else if (strncmp(token,USB_CRADLE_BACKHAUL,strlen(USB_CRADLE_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_second = BACKHAUL_TYPE_CRADLE;
      else if (strncmp(token,WLAN_BACKHAUL,strlen(WLAN_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_second = BACKHAUL_TYPE_AP_STA_ROUTER;
      else
        LOG_MSG_INFO1("Unknown preffered_backhaul_second read from xml!!",0,0,0);
    }
    ...
    if(token)
    {
      ...
      if (strncmp(token,WWAN_BACKHAUL,strlen(WWAN_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_third = BACKHAUL_TYPE_WWAN;
      else if (strncmp(token,USB_CRADLE_BACKHAUL,strlen(USB_CRADLE_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_third = BACKHAUL_TYPE_CRADLE;
      else if (strncmp(token,WLAN_BACKHAUL,strlen(WLAN_BACKHAUL)) == 0)
        QCMAP_Backhaul::preffered_backhaul_third = BACKHAUL_TYPE_AP_STA_ROUTER;
      else
        LOG_MSG_INFO1("Unknown preffered_backhaul_third read from xml!!",0,0,0);
    }
...
/*===========================================================================
  FUNCTION IsCradleOrStationActive
==========================================================================*/
/*!
@brief
...
*/
/*=========================================================================*/
boolean QCMAP_Backhaul::IsCradleOrStationActive()
{
  boolean cradle_wlan_status = false;
  QCMAP_Backhaul* QcMapBackhaulMgr = QCMAP_Backhaul::Get_Instance(false);
  if(QcMapBackhaulMgr)
  {
    cradle_wlan_status = (QcMapBackhaulMgr->current_backhaul == BACKHAUL_TYPE_CRADLE ||
   ...
   switch (QcMapBackhaulMgr->current_backhaul)
   {
     case BACKHAUL_TYPE_AP_STA_ROUTER:
       if (!QCMAP_Backhaul_WLAN::IsSTAAvailableV6())
       {
         ...
         else if (qcmap_cm_get_ipv6_state(qmi_err_num) == QCMAP_CM_V6_WAN_CONNECTED)
     ...
     case BACKHAUL_TYPE_CRADLE:
       if (!QCMAP_Backhaul_Cradle::IsCradleBackhaulAvailableV6())
       {
         /*Switch to APSTA or WWAN*/
         if (QCMAP_Backhaul_WLAN::IsSTAAvailableV6() &&
             (QcMapBackhaulMgr->ComparePriority(BACKHAUL_TYPE_AP_STA_ROUTER,BACKHAUL_TYPE_WWAN) ||
              qcmap_cm_get_ipv6_state(qmi_err_num) != QCMAP_CM_V6_WAN_CONNECTED))
     ...
     case BACKHAUL_TYPE_WWAN:
       ...
       {
         /*Switch to ccradle or APSTA*/
         if (QCMAP_Backhaul_WLAN::IsSTAAvailableV6() &&
             (QcMapBackhaulMgr->ComparePriority(BACKHAUL_TYPE_AP_STA_ROUTER,BACKHAUL_TYPE_CRADLE) ||
...
)
{
  ...
  else  if(QcMapBackhaulMgr->enable_ipv6)
  {
    ...
    else
    {
      if(!QcMapBackhaulMgr->ipv6_prefix_info.prefix_info_valid)
      {
        if(QcMapBackhaulWLANMgr || QcMapBackhaulCradleMgr || QcMapBackhaulWWANMgr)
        {
        ...
        if (NULL != QcMapBackhaulWLANMgr)
        {
          ...
          else
          {
            ...
            if (QcMapBackhaulMgr->current_backhaul == BACKHAUL_TYPE_AP_STA_ROUTER)
              QcMapBackhaulMgr->DeleteAllV6ExternalRoute(WLAN0);
    ...
    switch (QcMapBackhaulMgr->current_backhaul)
    {
      ...
      case BACKHAUL_TYPE_WWAN:
        if (qcmap_cm_get_state(qmi_err_num) != QCMAP_CM_WAN_CONNECTED)
        {
          ...
          else if (QCMAP_Backhaul_Cradle::IsCradleBackhaulAvailableV4())
            QcMapBackhaulCradleMgr->SwitchToCradleBackhaul();
...
boolean QCMAP_Backhaul::EnableIPV6Forwarding()
{
  ...
  if (QcMapBackhaulMgr->enable_ipv6 )
  {
    if( !QCMAP_Backhaul::IsCradleOrStationActive())
    {
         ...
         QcMapBackhaulWWANMgr && (QcMapBackhaulWWANMgr->ipv6_prefix_based_rules_added))
    {
      ...
      snprintf( command, MAX_COMMAND_STR_LEN,
               "ebtables -D INPUT -i wlan%d -p IPv6 --ip6-destination %s/%d -j DROP",
                QcMapMgr->ap_dev_num+1,ip6_addr,ipv6_prefix_info.prefix_len);
      ds_system_call( command, strlen(command));
      snprintf( command, MAX_COMMAND_STR_LEN,
    ...
    if ( (!QcMapBackhaulWWANMgr ||
         !QcMapBackhaulWWANMgr->prefix_delegation_activated) ||
         QCMAP_Backhaul::IsCradleOrStationActive())
    {
                    ...
                    QcMapBackhaulWLANMgr)
    {
      inet_ntop(AF_INET6, &QcMapBackhaulWLANMgr->ipv6_gateway_addr, ipv6_addr,
                               INET6_ADDRSTRLEN);
      snprintf(command, MAX_COMMAND_STR_LEN,
                 "ip -6 route add default via %s dev %s",ipv6_addr, WLAN0);
      ds_system_call(command, strlen(command));
    }
         ...
         (QcMapWLANMgr->wlan_cfg.interface[QCMAP_MSGR_INTF_GUEST_AP_INDEX].access_profile == QCMAP_MSGR_PROFILE_INTERNET_ONLY_V01) )
    {
           ...
           QcMapBackhaulMgr->ipv6_prefix_info.prefix_info_valid )
      {
        ...
        snprintf( command, MAX_COMMAND_STR_LEN,
                 ...
                 QcMapMgr->ap_dev_num+1,ip6_addr,QcMapBackhaulMgr->ipv6_prefix_info.prefix_len);
        ds_system_call( command, strlen(command));
        snprintf( command, MAX_COMMAND_STR_LEN,
  ...
  if (this->enable_ipv6 )
  {
    ...
    if ((ret == BACKHAUL_TYPE_AP_STA_ROUTER) && QcMapBackhaulWLANMgr)
    {
      inet_ntop(AF_INET6, &QcMapBackhaulWLANMgr->ipv6_gateway_addr, ip6_addr,
                               INET6_ADDRSTRLEN);
      snprintf(command, MAX_COMMAND_STR_LEN,
                 "ip -6 route del default via %s dev %s",ip6_addr, WLAN0);
      ds_system_call(command, strlen(command));
    }
    ...
    {
      if (!(this->DeleteGlobalipv6AddressInfo()))
      {
...
void QCMAP_Backhaul::AllowBackhaulAccessOnIface(ip_version_enum_type ip_vsn, char *devname)
{
  ...
  if (ip_vsn == IP_V4)
  {
    ...
    ==========================================================================*/
    LOG_MSG_INFO1("Allowing IPv4 Access for STA/Cradle Iface",0,0,0);
  ...
  else if (ip_vsn == IP_V6)
  {
    ...
    ==========================================================================*/
    LOG_MSG_INFO1("Allowing Bridge IPv6 Access for STA/Cradle Iface",0,0,0);
    ...
    Allow IPv4 Access between PPP Iface and Iface mentioned.
    ==========================================================================*/
    LOG_MSG_INFO1("Allowing PPP IPv6 Access for STA/Cradle Iface",0,0,0);
...
void QCMAP_Backhaul::AddV6ExternalRoute(qcmap_nl_addr_t* nl_addr, char* iface)
{
  ...
  if ((strncmp(iface, WLAN0, strlen(WLAN0)) == 0 && QcMapBackhaulMgr->current_backhaul == BACKHAUL_TYPE_AP_STA_ROUTER) ||
...
void QCMAP_Backhaul::DeleteAllV6ExternalRoute(char* iface)
{
  ...
  if ((strncmp(iface, WLAN0, strlen(WLAN0)) == 0 &&
       QcMapBackhaulMgr->current_backhaul == BACKHAUL_TYPE_AP_STA_ROUTER) ||
...
{
  static int v6sock_bridge = -1, v6sock_ecm = -1, v6sock_wlan = -1,iface_name_sockfd;
...
void QCMAP_Backhaul::AddIPv6DefaultRouteInfo(qcmap_nl_addr_t *nladdr)
{
  ...
  if ((strncmp(nladdr->iface_name, WLAN0, strlen(WLAN0)) == 0) &&
       QcMapBackhaulWLANMgr &&
