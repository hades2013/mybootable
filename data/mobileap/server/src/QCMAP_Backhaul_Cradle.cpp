...
)
{
  ...
  QCMAP_Backhaul_Cradle::GetSetCradleConfigFromXML(GET_VALUE,&(this->cradle_cfg.cradle_mode));
...
)
{
  ...
  if(NULL != QcMapBackhaulCradleMgr)
    *mode = QcMapBackhaulCradleMgr->cradle_cfg.cradle_mode;
    ...
    GetSetCradleConfigFromXML(GET_VALUE,mode);
...
)
{
  ...
  *qmi_err_num = QMI_ERR_NO_EFFECT_V01;
  ...
  QCMAP_Backhaul_Cradle::GetSetCradleConfigFromXML(GET_VALUE, &prev_mode);
  QCMAP_Backhaul_Cradle::GetSetCradleConfigFromXML(SET_VALUE, &mode);
    ...
    /* For JO there is no IPA. */
      ...
      switch (mode)
      {
         case QCMAP_MSGR_CRADLE_LAN_BRIDGE_V01:
           QCMAP_Backhaul_Cradle::SetECMCategory(LAN_CATEGORY, BRIDGE_MODE);
           break;
         case QCMAP_MSGR_CRADLE_LAN_ROUTER_V01:
           QCMAP_Backhaul_Cradle::SetECMCategory(LAN_CATEGORY, ROUTER_MODE);
           break;
         case QCMAP_MSGR_CRADLE_WAN_BRIDGE_V01:
           QCMAP_Backhaul_Cradle::SetECMCategory(LAN_CATEGORY, BRIDGE_MODE);
           break;
         case QCMAP_MSGR_CRADLE_WAN_ROUTER_V01:
           QCMAP_Backhaul_Cradle::SetECMCategory(WAN_CATEGORY, ROUTER_MODE);
           break;
         default:
    ...
    if ( true == ret )
    {
      ...
      else if (QCMAP_Backhaul_Cradle::IsCradleWANBridgeActivated() == true)/* Brige Mode Activated. Start DHCP Relay */
      {
            ...
            QcMapBackhaulMgr->current_backhaul != NO_BACKHAUL)
        {
          ...
          /*Not taking care of AP-STA Bridge mode assuming it would have been
          de-activated properly before moving to Cradle WAN Bridge Mode*/
        }
        ...
        (QCMAP_Backhaul_WLAN::IsAPSTABridgeActivated() == false))
    {
          ...
          QcMapBackhaulMgr->current_backhaul != NO_BACKHAUL)
      {
            ...
            QCMAP_Backhaul_WLAN::IsSTAAvailableV6())
        {
          ...
          if (QcMapBackhaulMgr->current_backhaul == BACKHAUL_TYPE_AP_STA_ROUTER)
          {
            QcMapBackhaulMgr->DeleteAllV6ExternalRoute(WLAN_IFACE);
            QcMapBackhaulMgr->ClearIfaceRelatedRulesForV6(QcMapBackhaulWLANMgr->apsta_cfg.sta_interface);
        }
      }
      }
...
void QCMAP_Backhaul_Cradle::ProcessCradleDisAssoc()
{
  ...
  QcMapBackhaulMgr->StopDHCPCD(ECM_IFACE);
...
*/
/*====================================================*/
boolean QCMAP_Backhaul_Cradle::SetECMCategory(char* category, char* mode)
{
...
/*=====================================================
  FUNCTION GetSetCradleConfigFromXML
======================================================*/
/*!
@brief
  Get and sets the Backhaul Cradle Config from XML.
...
static boolean QCMAP_Backhaul_Cradle::GetSetCradleConfigFromXML
(
  qcmap_action_type action,
  qcmap_msgr_cradle_mode_v01 *cradle_mode
)
{
  ...
  if(action == GET_VALUE)
  {
    *cradle_mode = atoi(root.child(CradleMode_TAG).child_value());
  }
  else
  {
    snprintf(data, sizeof(data), "%d", *cradle_mode);
    root.child(CradleMode_TAG).text() = data;
    QcMapMgr->WriteConfigToXML(UPDATE_MOBILEAP_XML,&xml_file);
  }
  LOG_MSG_INFO1("Cradle mode: %d Action: %d", *cradle_mode, action, 0);
