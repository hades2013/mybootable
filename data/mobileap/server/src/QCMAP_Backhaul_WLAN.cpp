...
)
{
  ...
  if (xml_path)
  {
    ...
    if (ReadAPSTAConfigFromXML())
    {
      ...
      this->apsta_cfg.ap_sta_bridge_sta_dhcp_running=false;
      strlcpy(this->apsta_cfg.sta_interface,WLAN_IFACE,QCMAP_MSGR_INTF_LEN);
      return;
    }
...
QCMAP_Backhaul_WLAN::~QCMAP_Backhaul_WLAN()
{
  ...
  if(!QcMapBackhaulCradleMgr && !QcMapBackhaulWWANMgr)
...
)
{
  ...
  else
  {
    ...
    /* Run wpa_supplicant for sta interface with specified config */
      if (strncmp(QcMapWLANMgr->wlan_cfg.module,AR6003,strlen(AR6003)) == 0)
    {
...
)
{
  ...
  {
    if ( QcMapWLANMgr->wifi_mode != QCMAP_MSGR_WLAN_MODE_AP_STA_V01 ||
          QcMapMgr->sta_iface_index != QCMAP_MSGR_INTF_STATION_INDEX || !QcMapBackhaulWLANMgr)
  {
  ...
  /* Run wpa_supplicant for sta interface with specified config */
    if( strncmp(QcMapWLANMgr->wlan_cfg.module,AR6003,strlen(AR6003)) == 0 )
  {
      snprintf( command, MAX_COMMAND_STR_LEN, "wpa_supplicant -i %s -Dar6003 -B -c %s",QcMapBackhaulWLANMgr->apsta_cfg.sta_interface,
                QcMapBackhaulWLANMgr->path_to_supplicant_conf);
  }
  else
  {
...
boolean QCMAP_Backhaul_WLAN::EnableStaMode()
{
  ...
  if( strncmp(QcMapWLANMgr->wlan_cfg.module,AR6003,strlen(AR6003)) == 0 )
  {
    snprintf( command, MAX_COMMAND_STR_LEN, "wpa_supplicant -i %s -Dar6003 -B -c %s",this->apsta_cfg.sta_interface,
          ...
          QCMAP_Backhaul_Cradle::IsCradleWANBridgeActivated() == false)
      {
      ...
      else if (QCMAP_Backhaul_WLAN::IsAPSTABridgeActivated() == true)/* Brige Mode Activated. Start DHCP Relay */
        {
          ...
          else
          {
            ...
            QCMAP_Backhaul_Cradle::GetSetCradleConfigFromXML(GET_VALUE, &mode);
...
void QCMAP_Backhaul_WLAN::EnterDefaultIPRulesForBridgeMode(void)
{
  ...
  snprintf(command, MAX_COMMAND_STR_LEN,
                   ...
                   QcMapMgr->ap_dev_num, inet_ntoa(this->ap_sta_bridge.bridge_def_gw), netmask, this->ap_sta_bridge.bridge_mac[0], this->ap_sta_bridge.bridge_mac[1],
...
void QCMAP_Backhaul_WLAN::EnterDefaultMACRulesForBridgeMode(void)
{
  ...
  snprintf(command, MAX_COMMAND_STR_LEN,"ebtables -t nat -A POSTROUTING -o wlan%d -j snat --to-source %02x:%02x:%02x:%02x:%02x:%02x",
                   QcMapMgr->ap_dev_num, this->ap_sta_bridge.ap_mac[0],this->ap_sta_bridge.ap_mac[1],this->ap_sta_bridge.ap_mac[2],
...
void QCMAP_Backhaul_WLAN::ProcessStaAssocForWLANBridge(void *cb_user_data)
{
  ...
  snprintf(ap_intf_name, (DSI_CALL_INFO_DEVICE_NAME_MAX_LEN+2), "wlan%d", QcMapMgr->ap_dev_num);
  /*Get all HW Address */
...
boolean QCMAP_Backhaul_WLAN::CleanV6ProxyEntries()
{
  ...
  while (node != NULL)
  {
    if (proxyInfo && (sys_time.tv_sec > (proxyInfo->time + 10)))
    {
