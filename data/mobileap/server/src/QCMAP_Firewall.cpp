  ...
  qmi_error_type_v01 *qmi_err_num)
{
  ...
  if (add_rule)
  {
    if (QcMapFirewall->firewall_config.firewall_pkts_allowed)
    {
      QcMapFirewall->DeleteConntrackEntryForAcceptIPv4FirewallEntries(firewall_entry,PS_IPPROTO_TCP);
      QcMapFirewall->DeleteConntrackEntryForAcceptIPv4FirewallEntries(firewall_entry,PS_IPPROTO_UDP);
    }
    else
    {
      if (next_hdr_prot == 0)
        QcMapFirewall->DeleteConntrackEntryForDropIPv4FirewallEntries(firewall_entry,PS_IPPROTO_TCP_UDP);
      else
        QcMapFirewall->DeleteConntrackEntryForDropIPv4FirewallEntries(firewall_entry,next_hdr_prot);
    }
  }
  ...
  qmi_error_type_v01 *qmi_err_num)
{
  ...
  if ( add_rule )
  {
    if ( QcMapFirewall->firewall_config.firewall_pkts_allowed )
    {
      QcMapFirewall->DeleteConntrackEntryForAcceptIPv6FirewallEntries(firewall_entry,PS_IPPROTO_TCP);
      QcMapFirewall->DeleteConntrackEntryForAcceptIPv6FirewallEntries(firewall_entry,PS_IPPROTO_UDP);
    }
    else if (!QcMapFirewall->firewall_config.firewall_pkts_allowed)
    {
      if (next_hdr_prot == 0)
      {
        QcMapFirewall->DeleteConntrackEntryForDropIPv6FirewallEntries( firewall_entry,PS_IPPROTO_TCP_UDP);
      }
      else
      {
        QcMapFirewall->DeleteConntrackEntryForDropIPv6FirewallEntries( firewall_entry,next_hdr_prot);
      }
    }
  }
...
void QCMAP_Firewall::CleanIPv4MangleTable(void)
{
  ...
  firewallList = &(this->firewall_config.extd_firewall_entries);
...
)
{
  ...
  snprintf(command, MAX_COMMAND_STR_LEN,"conntrack -L -f ipv6 | cut -f1,9,11,12 -d ' ' | grep -v dport=53 ");
