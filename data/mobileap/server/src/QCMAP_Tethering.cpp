...
/*------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR("Can't enable DUN/ETH/cradle wan bride when in AP-STA Brigde Mode",0,0,0);
...
------------------------------------------------------------------------*/
  ...
  else if ( tethered_link == QCMAP_QTI_TETHERED_LINK_ECM )
  {
    ...
    snprintf(command, MAX_COMMAND_STR_LEN, "echo 0 > /proc/sys/net/ipv6/conf/%s/disable_ipv6",
             ECM_IFACE);
    ds_system_call(command, strlen(command));
    if (mode == QCMAP_MSGR_CRADLE_WAN_ROUTER_V01)
    {
      if ((boolean)atoi(enablev6))
      {
        QCMAP_Backhaul::JoinMulticastGroup(QCMAP_V6_SOCK_ECM, ECM_IFACE);
      ...
      else
      {
        ...
        snprintf(command, MAX_COMMAND_STR_LEN, "echo 1 > /proc/sys/net/ipv6/conf/%s/accept_ra_defrtr",
                 ECM_IFACE);
        ds_system_call(command, strlen(command));
      }
    }
  ...
  if (tethered_link == QCMAP_QTI_TETHERED_LINK_RNDIS ||
      tethered_link == QCMAP_QTI_TETHERED_LINK_ETH ||
      ((mode == QCMAP_MSGR_CRADLE_LAN_BRIDGE_V01 || mode == QCMAP_MSGR_CRADLE_LAN_ROUTER_V01)
       && tethered_link == QCMAP_QTI_TETHERED_LINK_ECM))
  {
    /*------------------------------------------------------------------------
  ...
  else //WAN Mode
  {
    ...
    case QCMAP_MSGR_CRADLE_WAN_ROUTER_V01:
      ...
      if((boolean)atoi(enable))
      {
        snprintf( command, MAX_COMMAND_STR_LEN, "dhcpcd %s -t 0 -o\
                  domain_name_servers --noipv4ll -b -G",ECM_IFACE);
        ds_system_call(command, strlen(command));
    ...
    case QCMAP_MSGR_CRADLE_WAN_BRIDGE_V01:
      ...
      break;
    default:
        LOG_MSG_ERROR("Unsupported Cradle Mode %d, in usb mode %d",mode, tethered_link, 0);
        delete QcMapBackhaulCradleMgr;
    }
  }

  if (tethered_link == QCMAP_QTI_TETHERED_LINK_RNDIS ||
      (( mode == QCMAP_MSGR_CRADLE_LAN_BRIDGE_V01 ||
         mode == QCMAP_MSGR_CRADLE_LAN_ROUTER_V01 )
       && tethered_link == QCMAP_QTI_TETHERED_LINK_ECM))
  {
    /*Populating CDI based on bridge fdb info and neighbor table*/
    if ( tethered_link == QCMAP_QTI_TETHERED_LINK_RNDIS )
    {
      strlcpy ( iface, RNDIS_IFACE , strlen (RNDIS_IFACE));
    }
    else if( tethered_link == QCMAP_QTI_TETHERED_LINK_ECM )
    {
      strlcpy ( iface, ECM_IFACE , strlen (ECM_IFACE));
    }
