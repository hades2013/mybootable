...
#define STA_MODE_SUPPLICANT_FILE "/tmp/wpa_supplicant.conf"
...
QCMAP_WLAN::QCMAP_WLAN()
{
  ...
  QcMapMgr->strtolower(this->wlan_cfg.module);
  ...
  //after xml read
  strlcpy(this->wlan_cfg.module, MODULE_NAME, QCMAP_CM_MAX_FILE_LEN);
...
*/
/*====================================================================*/
boolean Associatewlantobridge(int wlanindex)
{
  char command[MAX_COMMAND_STR_LEN];
  int ret;
  int retry = 0;
...
*/
/*====================================================================*/
void Disassociatewlantobridge(int wlanindex)
{
  char command[MAX_COMMAND_STR_LEN];
...
static void FillinWLANStatusIND(void)
{
  ...
  strlcpy(QcMapWLANMgr->current_wlan_status.wlan_state[0].wlan_iface_name,
                             WLAN0, QCMAP_MSGR_DEVICE_NAME_MAX_V01);
  strlcpy(QcMapWLANMgr->current_wlan_status.wlan_state[1].wlan_iface_name,
                             WLAN1, QCMAP_MSGR_DEVICE_NAME_MAX_V01);
                         ...
                         boolean privileged_client )
{
  ...
  /* If STA Only Mode is enabled; disable it */
  if (QcMapWLANMgr && QcMapWLANMgr->is_sta_only_mode_enabled)
  {
    ...
    snprintf(command, MAX_COMMAND_STR_LEN, "rm -rf %s", STA_MODE_SUPPLICANT_FILE);
    ds_system_call(command, strlen(command));
  ...
  * tied to mobile ap handle. */
  ...
  if (station_config.ap_sta_bridge_mode &&
      ...
      (QcMapBackhaulCradleMgr->IsCradleWANBridgeActivated() == true))
  {
    LOG_MSG_ERROR("Cannot enable AP-STA Bridge mode when in Cradle WAN Bridge Mode.", 0, 0, 0);
    *qmi_err_num = QMI_ERR_NO_EFFECT_V01;
    return false;
  }
  ...
  QcMapMgr->sta_iface_index = QCMAP_MSGR_INTF_STATION_INDEX;
  /* Set AP device index. */
  QcMapMgr->ap_dev_num = 0;
  ...
  LOG_MSG_INFO1( "ap_dev_num: %d\n",QcMapMgr->ap_dev_num, 0, 0);

  // Enable the wlan module
  QcMapWLANMgr->EnableWLANModule(devmode,enable_wlan_onbootup);
...
void QCMAP_WLAN::EnableWLANModule
(
char *wlanmode,
boolean enable_wlan_onbootup
)
{
  ...
  * service availablity, we are spending time stopping hostapd */
  ...
  for ( i=0, dev = QcMapMgr->ap_dev_num; i < QcMapMgr->sta_iface_index;i++,dev++)
  {
    if ( !this->wlan_cfg.interface[i].enable )
      break;
                          ...
                          boolean privileged_client )
{
  ...
  QcMapWLANMgr->DisableWLANModule();
      ...
      || (QcMapBackhaulMgr->current_backhaul == BACKHAUL_TYPE_AP_STA_BRIDGE)))
  {
    ...
    if (QCMAP_Backhaul_Cradle::IsCradleWANBridgeActivated() == false)
    {
      if (((QCMAP_Backhaul_Cradle::IsCradleBackhaulAvailableV4() ||
            ...
            QcMapBackhaulMgr->ComparePriority(BACKHAUL_TYPE_CRADLE,BACKHAUL_TYPE_WWAN)) ||
    ...
    else if (QcMapBackhaulMgr->current_backhaul == BACKHAUL_TYPE_WWAN &&
             prev_wifi_mode != QCMAP_MSGR_WLAN_MODE_AP_STA_V01 &&
             qcmap_cm_get_ipv6_state(qmi_err_num) == QCMAP_CM_V6_WAN_CONNECTED )
  {
    /* Just Restart RADISH. */
    QcMapBackhaulMgr->StopRadish();
...
*/
/*====================================================================*/
void QCMAP_WLAN::DisableWLANModule(void)
{
  char command[MAX_COMMAND_STR_LEN];
  struct in_addr addr;
  char a5_ip[QCMAP_LAN_MAX_IPV4_ADDR_SIZE];
  ...
  snprintf(command, MAX_COMMAND_STR_LEN, "brctl delif %s wlan%d",
  BRIDGE_IFACE, QcMapMgr->ap_dev_num);
  ds_system_call(command, strlen(command));
  if ( this->wifi_mode == QCMAP_MSGR_WLAN_MODE_AP_AP_V01 )
  {
    snprintf(command, MAX_COMMAND_STR_LEN, "brctl delif %s wlan%d",
    BRIDGE_IFACE, QcMapMgr->ap_dev_num+1);
    ds_system_call(command, strlen(command));
  }
...
)
{
  ...
  switch ( ap_type )
  {
    case QCMAP_MSGR_PRIMARY_AP_V01:
    {
      ...
      this->StartHostAPD(QCMAP_MSGR_INTF_AP_INDEX, QcMapMgr->ap_dev_num);
      addr.s_addr = htonl(this->wlan_cfg
                   .interface[QCMAP_MSGR_INTF_GUEST_AP_INDEX].ll_subnet_mask);
      strlcpy(netmask, inet_ntoa(addr), 16);
      addr.s_addr = htonl(this->wlan_cfg
      ...
      snprintf(command, MAX_COMMAND_STR_LEN, "route del -net %s netmask %s dev\
        wlan%d", subnet, netmask, QcMapMgr->ap_dev_num);
      ds_system_call(command, strlen(command));
      if(!Associatewlantobridge(QcMapMgr->ap_dev_num))
      {
    ...
    case QCMAP_MSGR_GUEST_AP_V01:
    {
      ...
      this->StartHostAPD(QCMAP_MSGR_INTF_GUEST_AP_INDEX, QcMapMgr->ap_dev_num);
      addr.s_addr = htonl(this->wlan_cfg.
                    interface[QCMAP_MSGR_INTF_GUEST_AP_INDEX].ll_subnet_mask);
      strlcpy(netmask, inet_ntoa(addr), 16);
      addr.s_addr = htonl(this->wlan_cfg.
      ...
      strlcpy(subnet, inet_ntoa(addr), 16);
      snprintf(command, MAX_COMMAND_STR_LEN, "route del -net %s netmask %s\
                 dev wlan%d", subnet, netmask, QcMapMgr->ap_dev_num+1);
      ds_system_call(command, strlen(command));
      if(!Associatewlantobridge(QcMapMgr->ap_dev_num+1))
      {
        LOG_MSG_ERROR("Associatewlantobridge failed: %d.",0 , 0, 0);
        *qmi_err_num = QMI_ERR_NO_EFFECT_V01;
        return false;
    ...
    case QCMAP_MSGR_PRIMARY_AND_GUEST_AP_V01:
    {
      ...
      Proceed only upto STA iface index since STA is always last in the
      list (ap; ap,ap; ap,sta; ap,ap,sta supported). */
      for ( i=0, dev = QcMapMgr->ap_dev_num; i < QcMapMgr->sta_iface_index;
           i++, dev++ )
      {
        if ( !this->wlan_cfg.interface[i].enable )
          break;
...
)
{
  ...
  switch ( ap_type )
  {
    case QCMAP_MSGR_PRIMARY_AP_V01:
    {
      ...
      Disassociatewlantobridge(QcMapMgr->ap_dev_num);
      this->StopHostapdCli(ap_type);
      /* Stop the hostapd for Primary AP. */
      KillHostapd(ap_pid, QcMapMgr->ap_dev_num);
    ...
    case QCMAP_MSGR_GUEST_AP_V01:
    {
      ...
      Disassociatewlantobridge(QcMapMgr->ap_dev_num+1);
      this->StopHostapdCli(ap_type);
      /* Stop the hostapd for Guest AP. */
      KillHostapd(guest_ap_pid, QcMapMgr->ap_dev_num+1);
...
)
{
  ...
  switch ( ap_type )
  {
    ...
    case QCMAP_MSGR_GUEST_AP_V01:
    {
      ...
      this->StartHostAPD(QCMAP_MSGR_INTF_GUEST_AP_INDEX, QcMapMgr->ap_dev_num+1);
      addr.s_addr = htonl(this->wlan_cfg.
        interface[QCMAP_MSGR_INTF_GUEST_AP_INDEX].ll_subnet_mask);
      strlcpy(netmask, inet_ntoa(addr), 16);
      ...
      strlcpy(subnet, inet_ntoa(addr), 16);
      snprintf(command, MAX_COMMAND_STR_LEN, "route del -net %s netmask %s dev \
        wlan%d", subnet, netmask, QcMapMgr->ap_dev_num+1);
      ds_system_call(command, strlen(command));
      if(!Associatewlantobridge(QcMapMgr->ap_dev_num+1))
      {
        LOG_MSG_ERROR("Associatewlantobridge failed: %d.",0 , 0, 0);
    ...
    case QCMAP_MSGR_PRIMARY_AND_GUEST_AP_V01:
    {
      ...
      /* Bring up interfaces.
        ...
        Proceed only upto STA iface index since STA is always last in the
        list (ap; ap,ap; ap,sta; ap,ap,sta supported). */
      for ( i=0, dev = QcMapMgr->ap_dev_num; i < QcMapMgr->sta_iface_index; i++,
            dev++ )
      {
        if ( !this->wlan_cfg.interface[i].enable )
        break;
...
)
{
  ...
  switch ( ap_type )
  {
    case QCMAP_MSGR_PRIMARY_AP_V01:
    {
      ...
      "| awk '{print $1}'`",QcMapMgr->ap_dev_num);
      ds_system_call(command, strlen(command));
    }
    break;
    ...
    case QCMAP_MSGR_GUEST_AP_V01:
    {
      ...
      MAX_COMMAND_STR_LEN,
      "kill -15 `ps -ef | grep hostapd_cli | grep wlan%d"
      "| awk '{print $1}'`",QcMapMgr->ap_dev_num+1);
      ds_system_call(command, strlen(command));
    }
    break;
...
)
{
  ...
  switch ( ap_type )
  {
    case QCMAP_MSGR_PRIMARY_AP_V01:
    {
      if(!hostapdclicmd(QcMapMgr->ap_dev_num))
      {
    ...
    case QCMAP_MSGR_GUEST_AP_V01:
    {
      if(!hostapdclicmd(QcMapMgr->ap_dev_num+1))
      {
        LOG_MSG_ERROR("QCMAP_ConnectionManager::start hostapd_cli failed %d",ap_type, 0, 0);
      }
    }
...
boolean QCMAP_WLAN::StopHostAPD(void)
{
  ...
  switch (this->wifi_mode)
  {
    ...
    ds_system_call(command, strlen(command));
    KillHostapd(0, QcMapMgr->ap_dev_num);
    break;
    ...
    KillHostapd(0, QcMapMgr->ap_dev_num+1);
    snprintf( command, MAX_COMMAND_STR_LEN,"rm %s",HOSTAPD_PID_FILE);
    ds_system_call(command, strlen(command));
...
void QCMAP_WLAN::InstallGuestAPAccessRules(void)
{
  ...
  fprintf( access_fd, "ebtables -A INPUT -i wlan%d -p IPv4 --ip-destination\
    %s/%s -j DROP \n",QcMapMgr->ap_dev_num+1,a5_ip,netmask);
  fprintf( access_fd,"ebtables -I INPUT -i wlan%d -p IPv4 --ip-proto tcp\
      ...
      QcMapBackhaulCradleMgr|| QcMapBackhaulWLANMgr ))
  {
      ...
      -j DROP \n",QcMapMgr->ap_dev_num+1, publicIpAddr);
  ...
  {
    fprintf( access_fd, "ebtables -A INPUT -i wlan%d -p IPv6 --ip6-destination\
      %s/%d -j DROP \n",QcMapMgr->ap_dev_num+1,prefix_v6,ipv6_prefix_info_ptr->prefix_len);
...
boolean CheckHostapd(int omit_pid)
{
  ...
  LOG_MSG_ERROR("hostapd not running!! \n", 0, 0, 0);
  return false;
}
...
boolean QCMAP_WLAN::IsSupplicantRunning(void)
{
  ...
  LOG_MSG_ERROR("wpa_supplicant not running!! \n",0,0,0);
  return false;
}
...
boolean QCMAP_WLAN::ReadWLANConfigFromXML()
{
  ...
  strlcpy(this->wlan_cfg.module, root.child(Module_TAG).child_value(),\
...
boolean QCMAP_WLAN::SetIPAWLANMode(int mode)
{
  ...
  snprintf(guest_iface, DSI_CALL_INFO_DEVICE_NAME_MAX_LEN + 2, "wlan%d", QcMapMgr->ap_dev_num+1);
...
boolean QCMAP_WLAN::EnableSTAOnlyMode(qmi_error_type_v01 *qmi_err_num)
{
  ...
  LOG_MSG_INFO1("Wlan card brought up \n", 0,0,0);


  snprintf(command, MAX_COMMAND_STR_LEN,
              "echo \"ctrl_interface=\/var\/run\/wpa_supplicant\" > %s",
               STA_MODE_SUPPLICANT_FILE);
  ds_system_call( command, strlen(command));
  ...
  {
    LOG_MSG_ERROR("Could not start wpa_supplicant \n",0,0,0);
...
boolean QCMAP_WLAN::DisableSTAOnlyMode(qmi_error_type_v01 *qmi_err_num)
{
  ...
  {
    LOG_MSG_ERROR("\n WLAN is not enabled in STAOnly Mode.\n",0,0,0);
    *qmi_err_num = QMI_ERR_NO_EFFECT_V01;
    return false;
  }
  else if (!QcMapWLANMgr)
  {
    LOG_MSG_ERROR("\n WLAN is already disabled", 0,0, 0);
    *qmi_err_num = QMI_ERR_NO_EFFECT_V01;
    return false;
  }
  ...
  LOG_MSG_INFO1("Wlan card brought down \n",0,0,0);
...
void QCMAP_WLAN::SendWLANStatusIND(void)
{
  ...
  {
    LOG_MSG_ERROR("Invalid Objects!!! WLAN %p, QCMAP %p \n",QcMapWLANMgr, QcMapMgr, 0);
    return;
  }
