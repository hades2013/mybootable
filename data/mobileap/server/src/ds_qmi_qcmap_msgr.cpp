        ...
        else if ( i == qcmap_nl_sockfd )
        {
           ...
           else if((qcmap_nl_buffer->nl_iface == QCMAP_NL_WLAN) && (QcMapMgr))
           {
             ...
             else if (qcmap_nl_buffer->nl_event == QCMAP_NL_NEWADDR)
             {
               ...
               if (qcmap_nl_buffer->nl_addr.ip_vsn == IP_V4)
               {
                 ...
                 QCMAP_Backhaul_WLAN::ProcessAddrAssign(&qcmap_handle, WLAN0, IP_V4);
               ...
               {
                 LOG_MSG_INFO1("Received RTM_NEWADDR for IPv6 on wlan0",0,0,0);
                 QCMAP_Backhaul_WLAN::ProcessAddrAssign(&qcmap_handle, WLAN0, IP_V6);
                 ds_log_med("QCMAP_NL_NEWADDR on WLAN0 with IP Adddress \n");
               ...
               if (QcMapWLANMgr)
               {
                 ...
                 QcMapWLANMgr->SendWLANStatusIND();
                      ...
                      qcmap_nl_buffer->nl_addr.ip_vsn == IP_V6)
             {
               QCMAP_Backhaul::DeleteV6ExternalRoute(qcmap_nl_buffer->nl_addr.ip_v6_addr, WLAN0);
             }
             else
             {
           ...
           else if ((qcmap_nl_buffer->nl_iface == QCMAP_NL_ANY))
           {
             ...
             if (qcmap_nl_buffer->nl_event == QCMAP_NL_RA)
             {
               ...
               if (QcMapBackhaulCradleMgr || QcMapBackhaulWLANMgr)
               {
             ...
             {
               QCMAP_Backhaul::AddV6ExternalRoute(&(qcmap_nl_buffer->nl_addr), WLAN0);
             }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (req_ptr->first_priority_backhaul == QCMAP_MSGR_WWAN_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_FIRST_PREFFERED_BACKHAUL,
                  SET_VALUE,WWAN_BACKHAUL,sizeof(WWAN_BACKHAUL));
  else if (req_ptr->first_priority_backhaul == QCMAP_MSGR_USB_CRADLE_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_FIRST_PREFFERED_BACKHAUL,
                SET_VALUE,USB_CRADLE_BACKHAUL,sizeof(USB_CRADLE_BACKHAUL));
  else if (req_ptr->first_priority_backhaul == QCMAP_MSGR_WLAN_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_FIRST_PREFFERED_BACKHAUL,
                  SET_VALUE,WLAN_BACKHAUL,sizeof(WLAN_BACKHAUL));
  else
    error = true;

  if (req_ptr->second_priority_backhaul == QCMAP_MSGR_WWAN_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_SECOND_PREFFERED_BACKHAUL,
                  SET_VALUE,WWAN_BACKHAUL,sizeof(WWAN_BACKHAUL));
  else if (req_ptr->second_priority_backhaul == QCMAP_MSGR_USB_CRADLE_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_SECOND_PREFFERED_BACKHAUL,
                SET_VALUE,USB_CRADLE_BACKHAUL,sizeof(USB_CRADLE_BACKHAUL));
  else if (req_ptr->second_priority_backhaul == QCMAP_MSGR_WLAN_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_SECOND_PREFFERED_BACKHAUL,
                  SET_VALUE,WLAN_BACKHAUL,sizeof(WLAN_BACKHAUL));
  else
    error = true;

  if (req_ptr->third_priority_backhaul == QCMAP_MSGR_WWAN_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_THIRD_PREFFERED_BACKHAUL,
                  SET_VALUE,WWAN_BACKHAUL,sizeof(WWAN_BACKHAUL));
  else if (req_ptr->third_priority_backhaul == QCMAP_MSGR_USB_CRADLE_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_THIRD_PREFFERED_BACKHAUL,
                SET_VALUE,USB_CRADLE_BACKHAUL,sizeof(USB_CRADLE_BACKHAUL));
  else if (req_ptr->third_priority_backhaul == QCMAP_MSGR_WLAN_BACKHAUL_V01)
    QCMAP_Backhaul::GetSetBackhaulConfigFromXML(CONFIG_THIRD_PREFFERED_BACKHAUL,
                  SET_VALUE,WLAN_BACKHAUL,sizeof(WLAN_BACKHAUL));
  else
    error = true;
