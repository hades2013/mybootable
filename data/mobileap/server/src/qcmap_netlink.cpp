    ...
    switch(nlh->nlmsg_type)
    {
      ...
      case RTM_NEWNEIGH:
       ...
       if (((qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_RNDIS_INTERFACE) ==
        ...
        // If new Neigh is received on WLAN0 or WLAN1 interface and there is no IP Addresss.
...
----------------------------------------------------------------------*/
      case RTM_DELNEIGH:
        ...
        {
          if ( qcmap_nl_is_recv_on_dev(dev_name, WLAN0) == QCMAP_NL_SUCCESS)
          {
      ...
      case RTM_NEWADDR:
        ...
        if ((qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_WLAN_INTERFACE) ==
                                                           QCMAP_UTIL_FAILURE) &&
            (qcmap_nl_is_recv_on_dev(dev_name, BRIDGE_IFACE) ==
                                                           QCMAP_UTIL_FAILURE) &&
            (qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_ECM_INTERFACE) ==
        ...
        if (qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_WLAN_INTERFACE) ==
                                                           QCMAP_UTIL_SUCCESS)
          pktbuf.nl_iface = QCMAP_NL_WLAN;
        else if (qcmap_nl_is_recv_on_dev(dev_name, BRIDGE_IFACE) ==
                                                           QCMAP_UTIL_SUCCESS)
          pktbuf.nl_iface = QCMAP_NL_BRIDGE;
        else
          pktbuf.nl_iface = QCMAP_NL_USB;
