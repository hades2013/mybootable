...
LOCAL struct netmgr_client_state_s {
  ...
  netmgr_listener_info_t   usersock_netlink_socket;
...
netmgr_client_send_ping_msg(void)
{
  ...
  if( NULL == event_info ) {
    ...
    if( NETMGR_SUCCESS == result ) {
      ...
      if(NETMGR_SUCCESS != netmgr_nl_send_msg(netmgr_client_state.usersock_netlink_socket.ev_sk.sk_fd,
                                              (netmgrd_pid = ds_getpid(NETMGR_PROCESS_NAME)) >= 0 ? netmgrd_pid :NETMGR_NL_MULTICAST_SEND,
                                              buffer, buflen))
      {
...
)
{
  ...
  result = netmgr_nl_send_msg(netmgr_client_state.usersock_netlink_socket.ev_sk.sk_fd,
                              (netmgrd_pid = ds_getpid(NETMGR_PROCESS_NAME)) >= 0 ? netmgrd_pid : NETMGR_NL_MULTICAST_SEND,
...
netmgr_client_start_listener( void )
{
  ...
  if( netmgr_client_state.usersock_netlink_socket.running ) {
    netmgr_log_low("Listener already running\n");
    return NETMGR_SUCCESS;
  }
  ...
  /* Initialize NetLink socket interface and listening thread */
  if( NETMGR_SUCCESS !=
      netmgr_nl_listener_init( &netmgr_client_state.usersock_netlink_socket.sk_thrd_info,
                               netmgr_client_state.usersock_netlink_socket.sk_thrd_fdmap,
                               ds_arrsize(netmgr_client_state.usersock_netlink_socket.sk_thrd_fdmap),
                               &netmgr_client_state.usersock_netlink_socket.ev_sk,
                               NETLINK_USERSOCK,
                               0,
                               NULL) )
  {
  ...
  netmgr_client_state.usersock_netlink_socket.running = TRUE;
...
netmgr_client_stop_listener( void )
{
  ...
  if( !netmgr_client_state.usersock_netlink_socket.running ) {
    netmgr_log_low("Usersock Listener not running\n");
    return NETMGR_SUCCESS;
  }

  /* Teardown Usersock NetLink socket interface and listening thread */
  if( NETMGR_SUCCESS !=
      netmgr_nl_listener_teardown( &netmgr_client_state.usersock_netlink_socket.sk_thrd_info,
                                   &netmgr_client_state.usersock_netlink_socket.ev_sk ) )
  {
    NETMGR_ABORT("netmgr_client_stop_listener: Error on netmgr_nl_init\n");
    return NETMGR_FAILURE;
  ...
  netmgr_client_state.usersock_netlink_socket.running = FALSE;
...
netmgr_client_thread_wait( void )
{
  ...
  pthread_t usersock_netlink_thread_id;

  NETMGR_LOG_FUNC_ENTRY;

  NETMGR_CLIENT_LOCK();
  generic_netlink_thread_id = netmgr_client_state.generic_netlink_socket.sk_thrd_info.thrd;
  usersock_netlink_thread_id = netmgr_client_state.usersock_netlink_socket.sk_thrd_info.thrd;
  NETMGR_CLIENT_UNLOCK();
  ...
  if( 0 != pthread_join( usersock_netlink_thread_id, NULL ) ) {
    NETMGR_ABORT("netmgr_client_thread_wait: pthread_join failed\n");
  }
