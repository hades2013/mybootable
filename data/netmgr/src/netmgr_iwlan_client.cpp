  ...
  do
  {
    ...
    if (NET_WLAN == netConfig.net.type)
    {
                      ...
                      sizeof(netConfig.ip6.s6_addr)))
      {
        ...
        /* Due to new routing changes if V6 address is not
         * assigned to wlan interface the kernel will not
         * add a default route in the routing table which
         * will impact V6 data calls. If V6 address is
         * available, then we need not add this default
         * route */
        v6AddrAvail = true;
      }
      ...
      /* Check if V6 address was assigned to wlan interface, if not
       * we need to add the default route */
      if (false == v6AddrAvail)
      {
        NETMGR_LOG_MED("OnNetConfigChange: Install V6 default route");
        netmgr_kif_add_wlan_default_route(AF_INET6);
      }
    }
    /* If WLAN was last connected but not anymore, send a DISCONNECT */
    else if (NET_WLAN == lastNetType)
    {
      NETMGR_LOG_ERR("OnNetConfigChange: Propagating WLAN disconnect event");

      /* Reset v6 address flag */
      v6AddrAvail = false;
