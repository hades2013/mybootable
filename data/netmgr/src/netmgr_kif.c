...
LOCAL int
netmgr_kif_disable_forwarding(int ip_family, boolean force);
...
netmgr_kif_set_mtu(int link)
{
  ...
  if( NETMGR_FAILURE ==
    ...
    netmgr_log_high( "%s() assigned MTU %d on %s\n", __func__, mtu,netmgr_kif_get_name(link) );
  }
...
)
{
  ...
  std_strlcpy(ext_ioctl.u.if_name, rmnet_data_dev, IFNAMSIZ);
...
)
{
  ...
  if (frag_low_mem > 0)
  {
    ...
    memset(cmd, 0x0, NETMGR_KIF_MAX_COMMAND_LENGTH);
    std_strlprintf(cmd,
                   sizeof(cmd),
                   "echo %d > /proc/sys/net/ipv4/ipfrag_low_thresh",
                   frag_low_mem);

    if ((rc = ds_system_call(cmd, (unsigned int)std_strlen(cmd))) != NETMGR_SUCCESS)
    {
      netmgr_log_med("Failed to set ipv4 ipfrag_low_thresh %s : rc %d\n", cmd, rc);
    }
  }
  ...
  if (frag_high_mem > 0)
  {
    ...
    if ((rc = ds_system_call(cmd, (unsigned int)std_strlen(cmd))) != NETMGR_SUCCESS)
    {
      netmgr_log_med("Failed to set ipv4 ipfrag_high_thresh %s : rc %d\n", cmd, rc);
    }
  }
}
...
netmgr_kif_cfg_cnf( int link, const netmgr_nl_msg_t * nlmsg_info_ptr )
{
  ...
  {
    netmgr_log_high("netmgr_kif_cfg_cnf: fetching runtime settings for IPv6\n");
...
netmgr_kif_nl_recv_addr_msg( const netmgr_nl_msg_t * nlmsg_info_ptr )
{
  ...
  if( nlmsg_info_ptr->type == RTM_NEWADDR ) {
    ...
    */
    ...
    /* Check that the scope is global; ignore local adddresses */
    if( NETMGR_KIF_GLOBAL_SCOPE == ifaddr->ifa_scope ) {
      if (NETMGR_SUCCESS == netmgr_kif_is_addr_ll_iid_match(link, nlmsg_info_ptr)) {
        netmgr_log_high("address configuration update on link %d\n", link);
        netmgr_kif_cfg_cnf( link, nlmsg_info_ptr );
      }
    }
        ...
        if( TRUE == netmgr_kif_cfg.iwlan.is_v4_audio_forwarding_rules_installed)
        {
          ...
          if (NETMGR_SUCCESS != rc)
          {
            netmgr_log_err("%s(): Failed to remove dynamic audio rules for V4!\n",
                           __func__);
            netmgr_l2s_msg(netmgr_l2s_invalid_link,"IPV4 audio rule\\nremove failed");
            rc = NETMGR_FAILURE;
            break;
          }
          ...
          netmgr_l2s_msg(netmgr_l2s_invalid_link,"IPV4 audio rules removed");
        }
        else
        {
        ...
        if( TRUE == netmgr_kif_cfg.iwlan.is_v6_audio_forwarding_rules_installed )
        {
          ...
          if (NETMGR_SUCCESS != rc)
          {
            netmgr_log_err("%s(): Failed to remove dynamic audio rules for V6!\n",
                           __func__);
            netmgr_l2s_msg(netmgr_l2s_invalid_link,"IPV6 audio rule\\nremove failed");
            rc = NETMGR_FAILURE;
            break;
          }
          ...
          netmgr_l2s_msg(netmgr_l2s_invalid_link,"IPV6 audio rules removed");
        }
        else
        {
    ...
    switch(ip_family)
    {
      case AF_INET:
        ...
        else
        {
          ...
          netmgr_l2s_msg(netmgr_l2s_invalid_link,"IPV4 audio rules installed");
        }
        break;
      case AF_INET6:
        ...
        else
        {
          ...
          netmgr_l2s_msg(netmgr_l2s_invalid_link,"IPV6 audio rules installed");
        }
        break;
      default:
...
netmgr_kif_netlink_init( void )
{
  ...
  ** event indications for client multicast group */
  ...
  if(NETMGR_SUCCESS !=
     netmgr_nl_listener_init( &netmgr_kif_sk_usersock.sk_thrd_info,
                               netmgr_kif_sk_usersock.sk_thrd_fdmap,
                              ds_arrsize(netmgr_kif_sk_usersock.sk_thrd_fdmap),
                              &netmgr_kif_sk_usersock.ev_sk,
                              NETLINK_USERSOCK,
                              0,
...
)
{
  ...
  if (netmgr_main_cfg.data_format.ul_data_aggregation_protocol == NETMGR_WDA_UL_QMAPV3)
  {
...
/*!
@brief
  Update link configurations for all known links
@note
  - Dependencies
  - None
  - Side Effects
...
)
{
  ...
  for (i = 0; i < NETMGR_MAX_LINK; ++i)
  {
    ...
    {
      netmgr_kif_update_single_link_config(i);
    }
  }
}
...
)
{
  ...
  (void)netmgr_kif_disable_forwarding(AF_INET, force);
  (void)netmgr_kif_disable_forwarding(AF_INET6, force);
...
netmgr_kif_enable_forwarding(int ip_family)
{
  ...
  for (i = 0; i < num_rules; ++i)
  {
    if (ds_system_call2(forwarding_rules[i], (unsigned int)strlen(forwarding_rules[i]), function_debug))
    {
      netmgr_log_err("cmd: %s failed\n", forwarding_rules[i]);
      rc = NETMGR_FAILURE;
      break;
  ...
  if (NETMGR_SUCCESS == rc)
  {
    ...
    netmgr_log_med("netmgr_kif_enable_forwarding: enable complete for ip_family=%d\n",
                   ip_family);
  }
...
)
{
  ...
  if (FALSE == force && FALSE == NETMGR_KIF_GET_FORWARDING_ENABLED(ip_family))
  {
    netmgr_log_med("netmgr_kif_disable_forwarding: already disabled for ip_family=%d!\n",
                   ip_family);
    goto ret;
  }
  ...
  if (NETMGR_SUCCESS == rc)
  {
    ...
    netmgr_log_med("netmgr_kif_disable_forwarding: disable complete for ip_family=%d\n",
                   ip_family);
  }
...
netmgr_kif_iwlan_remove_jump_iptable_rules(int ip_family)
{
  ...
  netmgr_log_low("%s(): Removing iptables jump rules for family=%d",
                 __func__, ip_family);
  ...
  if (AF_INET == ip_family)
  {
         ...
         netmgr_kif_iwlan_process_rules(install_cmd, 1, FALSE))
    {
      netmgr_log_err("%s(): Failed to install rule for ip_family [%d]!",
                     __func__, ip_family);
      netmgr_l2s_msg(link, "install_iptable_rules_ex\\nV4 failed");
      goto bail;
    }
    netmgr_l2s_msg(link, "install_iptable_rules_ex\\nV4 complete");
  }
  ...
  if (AF_INET == ip_family)
  {
         ...
         netmgr_kif_iwlan_process_rules(remove_cmd, 1, FALSE))
    {
      netmgr_log_err("%s(): Failed to remove rule for ip_family [%d]!\n",
                     __func__, ip_family);
      netmgr_l2s_msg(link, "remove_iptable_rules_ex\\nV4 failed");
      goto bail;
    }
    netmgr_l2s_msg(link, "install_iptable_rules_ex\\nV4 complete");
  }
...
)
{
  ...
  if (ds_system_call2(cmd, (unsigned int)std_strlen(cmd), function_debug) != NETMGR_SUCCESS)
  {
    ...
    netmgr_l2s_format_msg(link, "remove_fwdg_rules\\nV%d failed",
                          (NETMGR_IPV4_ADDR == ip_addr->if_addr.type ? 4 : 6));
    goto bail;
  }
...
{                                                                                                                   \
  ...
  /* These rules will be installed at a higher priority than the legacy policies */                                 \
  /* 'proto' 58 - ICMPv6 */                                                                                         \
  ...
  /* src and dst address are generic to allow for both link-local and global addresses*/                            \
  cmd_length = snprintf(  xfrm_policy_cmd,                                                                          \
                          sizeof(xfrm_policy_cmd),                                                                  \
                          "ip xfrm policy add dir "#ipsec_dir" priority 10 src ::/0 dst ::/0 proto 58 type 135;");  \
                                                                                                                    \
  cmd_length += snprintf( (xfrm_policy_cmd + cmd_length),                                                           \
                          (sizeof(xfrm_policy_cmd) - cmd_length),                                                   \
                          "ip xfrm policy add dir "#ipsec_dir" priority 10 src ::/0 dst ::/0 proto 58 type 136");   \
                                                                                                                    \
  if (ds_system_call3(  xfrm_policy_cmd,                                                                            \
                        std_strlen(xfrm_policy_cmd),                                                                \
                        err_str,                                                                                    \
      ...
      std_strlen(err_str) != 0)                                                                                     \
  {                                                                                                                 \
    netmgr_log_err("Failed to install v6 special policies, err: %s", err_str);                                      \
    break;                                                                                                          \
  }                                                                                                                 \
} while (0)
...
{                                                                                                          \
  ...
  cmd_length = snprintf(  xfrm_policy_cmd,                                                                 \
                          sizeof(xfrm_policy_cmd),                                                         \
                          "ip xfrm policy delete dir "#ipsec_dir" src ::/0 dst ::/0 proto 58 type 135;");  \
                                                                                                           \
  cmd_length += snprintf(  (xfrm_policy_cmd + cmd_length),                                                 \
                           (sizeof(xfrm_policy_cmd) - cmd_length),                                         \
                           "ip xfrm policy delete dir "#ipsec_dir" src ::/0 dst ::/0 proto 58 type 136");  \
                                                                                                           \
  if (ds_system_call3(xfrm_policy_cmd,                                                                     \
                      std_strlen(xfrm_policy_cmd),                                                         \
                      err_str,                                                                             \
...
)
{
  ...
  for (i = 0; i < num_ts; i++)
  {
    ...
    if (NETMGR_SUCCESS !=
         netmgr_util_convert_qmi_ip_addr_range_to_str_range( &ts_list->ts[i].start_addr,
                                                             &ts_list->ts[i].end_addr,
                                                             addr_range,
                                                             sizeof(addr_range) ))
    {
    ...
    if (QMI_WDS_IPV4_TYPE == ts_list->ts[i].start_addr.family)
    {
      netmgr_ts[i].ip_type = AF_INET;
    }
    else if (QMI_WDS_IPV6_TYPE == ts_list->ts[i].start_addr.family)
    {
      netmgr_ts[i].ip_type = AF_INET6;
    }
    else
    {
      netmgr_log_err("%s(): Unknown address family!\n", __func__);
      goto bail;
    }
  }
...
)
{
  ...
  if (sa_config.param_mask & QMI_WDS_IPSEC_TS_LIST_PARAM_MASK)
  {
    /* Get the number of TSi selectors */
    sa->sa_tsi.num_ts = sa_config.ts_list.len;
  }

  if (sa_config.param_mask & QMI_WDS_IPSEC_TS_RESP_LIST_PARAM_MASK)
  {
    /* Get the number of TSr selectors */
    sa->sa_tsr.num_ts = sa_config.ts_list_resp.len;
  }
      ...
      NETMGR_TS_MODE_BOTH_ENABLED == sa->ts_mode)
  {
    ...
    if (NETMGR_SUCCESS != netmgr_kif_store_ts_params(&sa_config.ts_list,
                                                     sa->sa_tsi.ts_list,
                                                     sa->sa_tsi.num_ts))
    {
      ...
      NETMGR_TS_MODE_BOTH_ENABLED == sa->ts_mode)
  {
    ...
    if (NETMGR_SUCCESS != netmgr_kif_store_ts_params(&sa_config.ts_list_resp,
                                                     sa->sa_tsr.ts_list,
                                                     sa->sa_tsr.num_ts))
    {
...
/*===========================================================================
  FUNCTION  netmgr_kif_add_wlan_default_route
===========================================================================*/
/*!
@brief
  Adds a default route via wlan interface to wlan table
...
*/
/*=========================================================================*/
int netmgr_kif_add_wlan_default_route(int family)
{
  int rc = NETMGR_FAILURE, max_retry = 0;
  char wlan_route_cmd[NETMGR_MAX_COMMAND_LENGTH] = "";
  char error_string[NETMGR_MAX_COMMAND_LENGTH * 3] = "";

  /* Updated routing architecture requires the
   * addition of default routes in the wlan
   * specific table for data path to function
   * correctly */
  if (AF_INET == family)
  {
    snprintf(wlan_route_cmd,
             sizeof(wlan_route_cmd),
             "ip route add default dev wlan0 table wlan0");
  }
  else
  {
    snprintf(wlan_route_cmd,
             sizeof(wlan_route_cmd),
             "ip -6 route add default dev wlan0 table wlan0");
  }

  do
  {
    if (NETMGR_SUCCESS
          != ds_system_call3(wlan_route_cmd,
                            (unsigned int) std_strlen(wlan_route_cmd),
                             error_string,
                             sizeof(error_string),
                             function_debug))
    {
      netmgr_log_err("%s(): Failed to add default route for wlan...retrying");
    }

    /* Check the buffer for return status of the command */
    if (std_strlen(error_string) != 0)
    {
      netmgr_log_err("%s() [%s], retrying...", __func__, error_string);

      /* The failure is most likely due to non-availability
       * of "wlan0" table we will sleep for one second and
       * retry. We will bail out after three tries */
      max_retry++;

      /* Reset error buffer to empty */
      memset(error_string, 0, sizeof(error_string));
      sleep(1);
      continue;
    }

    /* If the error string is empty, then the command
     * was executed successfully */
    netmgr_log_low("%s(): Successfully added wlan default route", __func__);
    rc = NETMGR_SUCCESS;

    break;
  } while (NETMGR_KIF_WLAN_DEFAULT_ROUTE_ADD_MAX_RETRY > max_retry);

  if (NETMGR_KIF_WLAN_DEFAULT_ROUTE_ADD_MAX_RETRY == max_retry)
  {
    /* On builds with the older routing architecture, wlan
     * table does not exist and we also don't need to add
     * this interface specific route. Failure to add the
     * default route on older routing architecture will
     * not impact data path since the route will be looked
     * up in the main table */
    netmgr_log_err("%s(): Failed to add default route for wlan, max retry reached!",
                   __func__);
  }
...
netmgr_kif_iwlan_update_dynamic_config (int link, int ip_family)
{
  ...
  netmgr_log_med("netmgr_kif_iwlan_update_dynamic_config: link=%d, family=%d, "
                 "active_calls=%u, iptable_rules=%d, forwarding=%d, policy_routing=%d\n",
                 link,
                 ip_family,
                 active_calls,
...
netmgr_kif_iwlan_cleanup_dynamic_config(int link, int ip_family)
{
  ...
  if (0 == active_calls)
  {
    ...
    if (NETMGR_SUCCESS != netmgr_kif_disable_forwarding(ip_family, FALSE))
    {
