...
LOCAL void netmgr_main_process_target()
{
  ...
  else if (DS_TARGET_FUSION4_5_PCIE == target)
  {
    ...
    if (0 != netmgr_config_load(NETMGR_CONFIG_FILE, NETMGR_PROPERTY_DATA_TARGET_VALUE_FUSION4_5_PCIE))
    {
      ...
      netmgr_main_cfg.data_format.ul_data_aggregation_protocol = 5;
  ...
  else if (DS_TARGET_LE_TESLA == target)
  {
    netmgr_log_med("netmgrd: Tesla LE configuration, using configdb");
    if (0 != netmgr_config_load(NETMGR_CONFIG_FILE, NETMGR_PROPERTY_DATA_TARGET_VALUE_LE_9X45))
    {
      netmgr_log_err("%s: Unable to load Tesla LE config from xml", __func__);
      return;
    }
    netmgr_config_print();
  }
  ...
  {
    netmgr_log_med("target: %s not processed", target_str);
    return;
  }
...
)
{
  ...
  if (netmgr_main_get_dpm_enabled())
  {
    ...
    if(NETMGR_SUCCESS != netmgr_qmi_dpm_init())
    {
      ...
      netmgr_l2s_msg_seq(netmgr_l2s_invalid_link,"DPM init failed");
    }
    else
    {
      netmgr_log_med( "netmgr_qmi_dpm_init() successful. \n" );
      netmgr_l2s_msg_seq(netmgr_l2s_invalid_link,"DPM inited");
    }

    if (NETMGR_SUCCESS != netmgr_qmi_dpm_port_open())
    {
      netmgr_log_err("FATAL: netmgr_qmi_dpm_port_open() failed\n");
      netmgr_l2s_msg_seq(netmgr_l2s_invalid_link,"Port open failed");
    }
    else
    {
      netmgr_log_med( "netmgr_qmi_dpm_port_open() successful.\n" );
      netmgr_l2s_msg_seq(netmgr_l2s_invalid_link,"Port open successful")
    }
  }
  NETMGR_LOG_FUNC_EXIT;
}
...
)
{
  ...
  props->qos_format = netmgr_main_get_qos_enabled();
...
netmgr_main (int argc, char ** argv)
{
  ...
  if (IWLAN_DISABLE != netmgr_main_get_iwlan_enabled())
  {
    ...
    netmgr_qmi_rev_links_service_init(netmgr_main_cfg.data_format);
