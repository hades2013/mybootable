...
)
{
  ...
  char l2s_event_msg[netmgr_l2s_max_msg_size] = "";
  ...
  netmgr_l2s_format_msg(event_info->qmi_modem_service_ind.conn_id,
                        "rcvd %s",l2s_event_msg);
  for(i=0; i<NETMGR_MAX_LINK; i++)
  {
...
)
{
  ...
  if( mtu_ptr && (QMI_WDS_CURR_CALL_INFO_MTU & call_info.mask) ) {
    *mtu_ptr = call_info.mtu;
    netmgr_log_med("Link MTU: %d; Family: %d\n", *mtu_ptr, evt_ip_family );
  } else {
...
)
{
  ...
  switch( link_status ) {
    case QMI_WDS_PACKET_DATA_CONNECTED:
    {
      ...
      if(QMI_WDS_PKT_SRVC_IND_XLAT_CAP & ind_data->pkt_srvc_status.param_mask)
      {
         netmgr_qmi_cfg.links[link].wds_info.xlat_capable = ind_data->pkt_srvc_status.xlat_capable;
      }
      ...
      netmgr_l2s_format_msg(link,"WDS_CONNECTED IPV%d",
                           (ip_family == QMI_IP_FAMILY_PREF_IPV4)? 4 : 6);
    }
    break;
...
)
{
                                            ...
                                            &wda_response)) != QMI_NO_ERR)
  {
        ...
        (wda_request.qos_header_format_valid))
    {
      ...
      {
        netmgr_log_high("%s() Modem rejected dl_minimum_padding on (%s)",
                        __func__,
                        dev_name);
      }
    }
...
#ifdef FEATURE_DATA_IWLAN
    /* Delay the QMI service init for the reverse rmnet links
     * CND daemon can send WLAN available event to modem at
     * any time. As soon as modem receives this message it will
     * try to bringup the reverse interface which may happen
     * while netmgr is in the process of initializing the links
     * This may result in the incoming WDS_CONNECTED event on the
     * reverse link to be lost. By delaying the QMI client
     * allocation we can quickly setup the state machine and
     * allow the modem to gracefully fail in the error case
     * The modem can retry once the client is allocated */
    if (NETMGR_IS_REV_IP_TRANS_CONN_ID(qmi_cid)) {
      netmgr_log_med( "Delaying qmi init for link [%d]\n", i);
      continue;
    }
#endif /* FEATURE_DATA_IWLAN */
...
/*===========================================================================
  FUNCTION  netmgr_qmi_rev_links_service_init
===========================================================================*/
/*!
@brief
 Initializes the QMI Driver for the reverse rmnet links
...
/*=========================================================================*/
void
netmgr_qmi_rev_links_service_init(netmgr_data_format_t data_format)
{
  int i;
  char * qmi_cid;
  int qmi_err;
  ...
  netmgr_log_med( "netmgr_qmi_rev_links_service_init\n");
  ...
  for (i = 0; i < NETMGR_MAX_LINK; i++)
  {
    ...
    if (NULL == qmi_cid)
    {
      netmgr_log_err("Error in getting conn_id for link[%d]\n",
                     i);
      continue;
    }
    ...
    if (NETMGR_IS_REV_IP_TRANS_CONN_ID(qmi_cid))
    {
      ...
      /* If the property is NSWO only , then enable 8 forward ports and one local
               breakout port */
      if(NSWO_ONLY == netmgr_main_get_iwlan_enabled() &&
         !NETMGR_IS_REV_IP_TRANS_LOCAL_BRK_OUT_CONN_ID(qmi_cid))
      {
          netmgr_log_low( "Error on QMI connection init, "
                          "suppressing link [%d]\n", i );
          continue;
      }
