...
)
{
  ...
  length = (unsigned int)snprintf(cmd, maxlength, "iptables -t mangle -A %s_0x%08x.%d",
                    NETMGR_TC_CHAIN_NAME_PREFIX,
                    flow_id,
                    rule_id);
  ...
  length = (unsigned int)snprintf(cmd,
                    maxlength,
                    "iptables -t mangle -A %s_0x%08x.%d",
                    NETMGR_TC_CHAIN_NAME_PREFIX,
                    flow_id,
                    rule_id);
  if( length >= maxlength )
...
)
{
  ...
  /* Create a chain */
  memset( cmd, 0x0, sizeof(cmd) );
  snprintf(cmd, maxlength, "iptables -t mangle -N %s_0x%08x.%d",
           NETMGR_TC_CHAIN_NAME_PREFIX,
           flow_id,
           rule_id);
...
error:
  ...
  snprintf(cmd, sizeof(cmd), "iptables -t mangle -X %s_0x%08x.%d",
           NETMGR_TC_CHAIN_NAME_PREFIX,
           flow_id,
           rule_id);
...
)
{
  ...
  memset( cmd, 0x0, sizeof(cmd) );
  length = (unsigned int)snprintf(cmd, maxlength, "ip6tables -t mangle -A %s_0x%08x.%d",
                    NETMGR_TC_CHAIN_NAME_PREFIX,
                    flow_id,
                    rule_id);
...
)
{
  ...
  /* Create a chain */
  memset( cmd, 0x0, sizeof(cmd) );
  snprintf(cmd, maxlength, "ip6tables -t mangle -N %s_0x%08x.%d",
           NETMGR_TC_CHAIN_NAME_PREFIX,
           flow_id,
           rule_id);
...
error:
  /* Deleting the chain deletes all the rules.  This chain must not be referenced by any other rules. */
  snprintf(cmd, sizeof(cmd), "ip6tables -t mangle -X %s_0x%08x.%d",
           NETMGR_TC_CHAIN_NAME_PREFIX,
           flow_id,
           rule_id);
  (void) ds_system_call(cmd, (unsigned int)std_strlen(cmd));
