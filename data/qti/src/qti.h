...
#define QTI_LOG_FILE                 "/data/qti_logs.txt"
...
void qti_print_msg(const char *funcName, uint32 lineNum, const char*fmt, ...);

#define PRINT_MSG( level, fmtString, x, y, z)                         \
        if ( file_log_mode )                                         \
        {                                                            \
          qti_print_msg( __FUNCTION__, __LINE__, fmtString, x, y, z);\
        }\
        else\
        {\
...
#define PRINT_MSG_6( level, fmtString, a, b, c, d, e, f)              \
        if ( file_log_mode )\
        {\
          qti_print_msg( __FUNCTION__, __LINE__, fmtString, a, b, c, d, e, f);   \
        }\
        else\
        {\
