...
/*--------------------------------------------------------------------------*/
  while ( retry_count < QTI_FILE_NODE_OPEN_MAX_RETRY)
  {
    ...
    if(qti_rmnet_fd < 0)
    {
      LOG_MSG_ERROR("Failed to open peripheral device file."
                    " Retrying. Error %d", errno, 0, 0);
      sleep(ONE_SEC);
      retry_count++;
      continue;
    }
  ...
  {
    LOG_MSG_ERROR("Could not open device file. Abort. Errno %d",
                   errno, 0, 0);
    return QTI_FAILURE;
  }
}
...
/*-------------------------------------------------------------------------*/
  ...
  ret_val = qti_ph_file_open(rmnet_state->ph_iface[PH_DRIVER_TYPE_USB].ph_iface_device_file,
  ...
  ret_val = qti_ph_file_open(rmnet_state->ph_iface[PH_DRIVER_TYPE_MHI].ph_iface_device_file,
...
/*-----------------------------------------------------------------------*/
  ...
  strlcpy(ifr.ifr_name,
              rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_data_iface_name,
