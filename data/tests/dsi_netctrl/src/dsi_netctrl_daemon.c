...
/* Wewins code*/
void get_call_end_reason(dsi_net_call_info* call, int fd)
{
  dsi_ip_family_t ipf;
  dsi_ce_reason_t last_call_end_reason;
  if (!call)
  {
     return;
  }
  
  if (DSI_SUCCESS == dsi_get_call_end_reason(call->handle, &(last_call_end_reason), DSI_IP_FAMILY_V4))
  {
     char buf[256];
     snprintf(buf, sizeof(buf), "type: %d code: %d\n",
     last_call_end_reason.reason_type,
     last_call_end_reason.reason_code);
     WRITE_STR(fd, buf);
  }
  else
  {
     WRITE_STR(fd, "get call end reason failed.\n");
  }
...
void do_cmd(dsi_netctrl_test_cmd_info *cmd, int fd)
{
  ...
  case DSI_NETCTRL_TEST_CMD_GET_CALL_END_REASON:
  	
	call = find_call(cmd->name);
    if (!call)
    {
      WRITE_STR(fd, "Create call failed.  Call with that name Not found.\n");
      break;
    }

    get_call_end_reason(call, fd);
    break;
