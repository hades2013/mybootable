...
#define PID_FILE_LEN 26
#define PID_FILE "/var/run/diag_socket.pid"
#define PID_DIR "/var/run"
#else
#define PID_FILE_LEN	35
#define PID_FILE	"/sdcard/diag_logs/diag_socket_pid"
#define PID_DIR		"/sdcard/diag_logs"
#endif
...
int open_connection(struct diag_socket *sock)
{
	...
	sock->fd = socket(res->ai_family, SOCK_STREAM, 0);
	if (sock->fd == -1) {
...
static void adjust_permissions()
{
	...
	gid_t diag_groups[] = {AID_QCOM_DIAG, AID_SDCARD_R, AID_MEDIA_RW, AID_SDCARD_RW, AID_INET};
	...
	if (status < 0) {
		DIAG_LOGE("diag_socket_log: Error setting group id, errno: %d, Exiting ...\n", errno);
		goto fail_permissions;
	}

	/*
	 * Add qcom_diag and sdcard_r as supplemental groups so we can
	 * access /dev/diag and /storage. Add AID_MEDIA_RW so we can write to
	 * the external SD card. Add AID_INET for accessing sockets.
	 */
	status = setgroups(size, diag_groups);
	if (status == -1) {
		DIAG_LOGE("diag_socket_log: setgroups error, errno: %d, Exiting ...\n", errno);
		goto fail_permissions;
	}

	status = setuid(AID_SHELL);
	if (status < 0) {
		DIAG_LOGE("diag_socket_log: Error setting user id, errno: %d, Exiting ...\n", errno);
		goto fail_permissions;
	}
	return;
...
static void adjust_permissions()
{
	...
	size_t size = 0;
	uid_t uid;
	gid_t diag_groups[] = { LE_GID_SDCARD, LE_GID_SOCKET };
	size = sizeof(diag_groups)/sizeof(gid_t);
	...
	if (uid == 0) {
		...
		if (status < 0) {
			printf("diag_socket_log: Error setting group id, errno: %d, Exiting ...\n", errno);
			goto fail_permissions;
		}
		...
		status = setgroups(size, diag_groups);
		if (status == -1) {
			printf("diag_socket_log: setgroups error, errno: %d, Exiting ...\n", errno);
			goto fail_permissions;
		}
		...
		if (status < 0) {
			printf("diag_socket_log: Error setting user id, errno: %d, Exiting ...\n", errno);
			goto fail_permissions;
		}
	}
	return;
...
int main(int argc, char *argv[])
{
	...
	int pid, pid_fd;
	char pid_buff[10];
	char pid_file[PID_FILE_LEN] = PID_FILE;
